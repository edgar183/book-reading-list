...
class RegisterForm(FlaskForm):
    ...
    username = StringField('Username', validators=[DataRequired("User name must be from 2 to 20 characters long."), Length(min=2, max=20)])
    password = StringField('Password', validators=[DataRequired("Passwords must be at least 8 characters long."),Length(min=3, max=8)])
...
class LoginForm(FlaskForm):
    ...
    password = StringField('Password', validators=[DataRequired("Incorect Password")])
